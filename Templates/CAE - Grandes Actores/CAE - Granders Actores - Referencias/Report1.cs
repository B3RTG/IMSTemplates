...
namespace CAE___Granders_Actores___Referencias
{
    public partial class Report1
    {
        private void Hoja2_Startup(object sender, System.EventArgs e)
        {
            ...
            try
            {
                if (Globals.ThisWorkbook.StatusCorrect)
                {
                    if (this.paint_data())
                    {
                        if (this.format_data() && CAE___Granders_Actores___Referencias.Helppers.SetupHeaders(Globals.Data1, this))
                        {
            ...
            {
                oJob.ReportStatus.Message = eReport1.Message;
            }
        }
        ...
        public bool format_data()
        {
            ...
            //lvl 15 minimo
            List<int> GroupLimits = new List<int>();
            GroupLimits.AddRange(new int[] { 13 }); //limita con 14 y 13
            oRangeToAdd = getGroupRanges(14, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);

            // 8 y 13, mismo level --> 3 tambien es mismo lvl
            //lvl 13 que incluye a los 14
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 12, 6 }); //limita con 12 o 14
            oRangeToAdd = getGroupRanges(13, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);
            // lvl 8 que incluye a los 9 y limita con otros 7
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 7 });
            oRangeToAdd = getGroupRanges(8, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);
            //lvl 3 que incluye a los 4
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 2 });
            oRangeToAdd = getGroupRanges(3, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);


            // 2, 7 y 12 son del mismo nivel
            //grupos 2, limita con algun 6
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 6 });
            oRangeToAdd = getGroupRanges(2, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);
            //lvl 7
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 12, 6 });
            oRangeToAdd = getGroupRanges(7, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);
            //lvl 12 Problemas....
            GroupLimits.Clear();
            GroupLimits.AddRange(new int[] { 6 });
            oRangeToAdd = getGroupRanges(12, GroupLimits, AllCellsRange);
            oRangeGroups.AddRange(oRangeToAdd);
            ...
            {
                if ((oCell.Value == 1 || oCell.Value == 6) && PreviousParentRange != null)
                { //cambio de grupo
                ...
                else if (oCell.Value == 1 || oCell.Value == 6)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 8 || (oCell.Value == 7)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row == oCell.Row - 1)
                    { //skip
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == 8) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == 8 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 2 || (oCell.Value == 6)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row == oCell.Row - 1)
                    { //skip
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == 2) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == 2 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 3 || (oCell.Value == 2)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row == oCell.Row - 1)
                    { //skip
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == 3) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == 3 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 12 || oCell.Value == 6) && PreviousParentRange != null)
                { //cambio de grupo
                    Excel.Range toAdd = this.Range[this.Cells[PreviousParentRange.Row + 1, PreviousParentRange.Column], this.Cells[oCell.Row - 1, oCell.Column]];
                    oRangeGroups.Add(toAdd);
                    PreviousParentRange = oCell;
                }
                else if (oCell.Value == 12 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 7 || (oCell.Value == 12 || oCell.Value == 6)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row == oCell.Row - 1)
                    { //skip
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == 7) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == 7 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
            ...
            foreach (Excel.Range oCell in AllCellsRange.Cells)
            {
                if ((oCell.Value == 13 || (oCell.Value == 12)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row == oCell.Row - 1)
                    { //skip
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == 13) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == 13 && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
        ...
        public ArrayList getGroupRanges(int GroupCellNumber, List<int> GroupLimits, Excel.Range AllRangeCells)
        {            
            //bucle grupos lvl 1 y 6
            ArrayList oRangeGroups = new ArrayList();
            ...
            {
                if ((oCell.Value == GroupCellNumber || GroupLimits.Contains((int)oCell.Value)) && PreviousParentRange != null)
                { //cambio de grupo
                    if (PreviousParentRange.Row != oCell.Row - 1)
                    { //add range
                        Excel.Range toAdd = this.Range[this.Cells[PreviousParentRange.Row + 1, PreviousParentRange.Column], this.Cells[oCell.Row - 1, oCell.Column]];
                    ...
                    //PreviousParentRange = oCell;
                    if (oCell.Value == GroupCellNumber) PreviousParentRange = oCell;
                    else PreviousParentRange = null;
                }
                else if (oCell.Value == GroupCellNumber && PreviousParentRange == null)
                {
                    PreviousParentRange = oCell;
                }
        ...
        public bool paint_data()
        {
            ...
            CAE___Granders_Actores___Referencias.Data1 oData1 = Globals.Data1;
            ...
            */
            ...
            oRangeColumn.Copy();
            oRangeColumn.PasteSpecial(Excel.XlPasteType.xlPasteValues);
            ...
            */
            ...
            Helppers.fillEmptySpacesCells(this, this.Range["A4:AL" + (iMaxRow + 2).ToString()]);
